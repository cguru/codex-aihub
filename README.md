...
현재 `0.3.0` 버전은 데이터셋 검색·개수·상세 정보·가이드 조회, 정확한 파일 목록 확인, 선택한 파일 다운로드를 지원합니다. 전체 데이터 승인이 없어도 **샘플(경량) 데이터**가 제공되면 Codex가 로그인된 AI Hub 웹페이지에서 샘플을 받아 모델 실습에 사용할 수 있습니다.
...
샘플 데이터는 AI Hub 웹 로그인이 필요하면 로그인 페이지를 안내하고,
...
샘플(경량) 데이터를 내려받고 작은 모델을 만들 준비를 해줘.
```
...
| 구분 | 인증 | 플러그인의 처리 |
| --- | --- | --- |
| 샘플(경량) 데이터 | AI Hub 웹 로그인 | 로그인된 브라우저에서 데이터셋 페이지의 **샘플(경량) 데이터** 링크 사용 |
| 정식 API 파일 | `AIHUB_API_KEY` + 해당 데이터셋 다운로드 승인 | 파일 키와 용량을 먼저 확인한 뒤 `download_dataset_files`로 선택 다운로드 |

샘플 링크는 API 키만으로 받을 수 있는 파일이 아닙니다. Codex가 브라우저를 사용할 수 없으면 데이터셋 페이지를 열어 사용자가 샘플 버튼을 직접 누르도록 안내합니다. 브라우저 비밀번호나 쿠키를 Codex에 전달할 필요는 없습니다.

정식 데이터의 파일 목록이 보인다고 다운로드 승인이 완료된 것은 아닙니다. 승인되지 않은 데이터셋을 다운로드하려 하면 플러그인은 다음처럼 안내합니다.

```text
이 AI Hub 데이터셋의 다운로드 승인이 확인되지 않았습니다.
데이터셋 상세 페이지에서 다운로드를 눌러 이용 신청과 승인을 완료한 뒤 다시 시도하세요.
```

승인 절차를 우회하거나 전체 데이터셋을 자동 선택하지 않습니다.
