...
현재 `0.5.3` 버전은 데이터셋 검색·개수·상세 정보·가이드 조회, 정식 데이터 다운로드 승인 사전 확인, 정확한 파일 목록과 전체 용량 확인, 대상 드라이브 여유 공간 검사, 선택한 파일 다운로드를 지원합니다. 일반적인 데이터 다운로드나 모델 구축 요청은 승인과 저장 공간을 먼저 확인하며, **샘플(경량) 데이터는 사용자가 명시적으로 샘플 데이터를 요청했을 때만** 사용합니다.
...
codex-aihub 플러그인을 다시 설치해줘. 업데이트 전후 버전을 비교해서 알려주고,
기존 AIHUB_API_KEY 환경변수와 사용자 데이터는 변경하거나 삭제하지 마.
