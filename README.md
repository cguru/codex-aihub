...
현재 `0.5.0` 버전은 데이터셋 검색·개수·상세 정보·가이드 조회, 정식 데이터 다운로드 승인 사전 확인, 정확한 파일 목록과 전체 용량 확인, 대상 드라이브 여유 공간 검사, 선택한 파일 다운로드를 지원합니다. 일반적인 데이터 다운로드나 모델 구축 요청은 승인과 저장 공간을 먼저 확인하며, **샘플(경량) 데이터는 사용자가 명시적으로 샘플을 요청했을 때만** 사용합니다.
...
4. 승인이 확인되면 API 파일 목록의 정확한 전체 용량과 대상 드라이브 여유 공간을 먼저 비교합니다.
...
AI Hub 상세 페이지의 데이터 크기는 대략값일 수 있습니다. 플러그인은 API 파일 목록의 모든 바이트를 합산한 값을 실제 다운로드 계획의 기준으로 사용합니다.

예를 들어 `71362` 재활용품 분류 및 선별 데이터는 페이지 설명을 합산하면 약 2,876GB이지만, 현재 API 파일 126개의 합계는 정확히 `2,532,921,144,222`바이트, 약 `2.53TB(2.30TiB)`입니다. 플러그인은 이 차이를 구분해서 설명합니다.
